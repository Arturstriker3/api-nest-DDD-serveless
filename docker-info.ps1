Write-Host "Wealthcare API - Docker Information" -ForegroundColor Cyan
Write-Host "==================================" -ForegroundColor Cyan

Write-Host ""
Write-Host "Image Details:" -ForegroundColor Yellow
docker images wealthcare-api --format "table {{.Repository}}`t{{.Tag}}`t{{.Size}}`t{{.CreatedAt}}"

Write-Host ""
Write-Host "Image Layers:" -ForegroundColor Yellow
docker history wealthcare-api --format "table {{.CreatedBy}}`t{{.Size}}"
...
Write-Host "  Manual run:  docker run -p 3000:3000 wealthcare-api" -ForegroundColor White
...
$containerRunning = docker ps --filter "name=wealthcare-api" --format "{{.Names}}" 2>$null
if ($containerRunning) {
    docker stats wealthcare-api --no-stream 2>$null
} else {
